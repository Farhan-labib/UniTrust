...
class UniAdminCAgent(AriesAgent):
    def __init__(
        self,
        ident: str,
        http_port: int,
...
async def main(args):
    ...
    if os.path.exists("/.dockerenv"):  # We're running inside a Docker container
        ...
        # Set the correct endpoint to eliminate localhost errors - using port 8120 for uni_admin_c
        os.environ["AGENT_ENDPOINT"] = "http://172.17.0.1:8120"
    ...
    endpoint = os.environ.get('ENDPOINT', 'http://172.17.0.1:8120')
    ...
    uni_admin_c_agent = await create_agent_with_args(
        args,
        ident="uni_admin_c",
    ...
    try:
        log_status(
            ...
            + (
                f" (Wallet type: {uni_admin_c_agent.wallet_type})"
                if uni_admin_c_agent.wallet_type
                else ""
            )
        )
        
        agent = UniAdminCAgent(
            "uni_admin_c.agent",
            uni_admin_c_agent.start_port,
            uni_admin_c_agent.start_port + 1,
            genesis_data=uni_admin_c_agent.genesis_txns,
            genesis_txn_list=uni_admin_c_agent.genesis_txn_list,
            no_auto=uni_admin_c_agent.no_auto,
            tails_server_base_url=uni_admin_c_agent.tails_server_base_url,
            revocation=uni_admin_c_agent.revocation,
            timing=uni_admin_c_agent.show_timing,
            multitenant=uni_admin_c_agent.multitenant,
            mediation=uni_admin_c_agent.mediation,
            wallet_type=uni_admin_c_agent.wallet_type,
            seed=uni_admin_c_agent.seed,
            aip=uni_admin_c_agent.aip,
            endorser_role=uni_admin_c_agent.endorser_role,
            anoncreds_legacy_revocation=uni_admin_c_agent.anoncreds_legacy_revocation,
            log_file=uni_admin_c_agent.log_file,
            log_config=uni_admin_c_agent.log_config,
            log_level=uni_admin_c_agent.log_level,
            reuse_connections=uni_admin_c_agent.reuse_connections,
            multi_use_invitations=uni_admin_c_agent.multi_use_invitations,
            public_did_connections=uni_admin_c_agent.public_did_connections,
            extra_args=extra_args,
        )

        # Debug: Show exact port configuration
        log_msg(f"🔧 Admin Agent Port Configuration:")
        log_msg(f"   Start Port: {uni_admin_c_agent.start_port}")
        log_msg(f"   HTTP Port: {uni_admin_c_agent.start_port} (agent communication)")
        log_msg(f"   Admin API Port: {uni_admin_c_agent.start_port + 1}")
        log_msg(f"   Webhook Port: {uni_admin_c_agent.start_port + 2}")

        # Initialize the same way as registrar agent
        if uni_admin_c_agent.cred_type == CRED_FORMAT_INDY:
            uni_admin_c_agent.public_did = True
            await uni_admin_c_agent.initialize(the_agent=agent)
            log_msg("Admin agent initialized successfully")
        elif uni_admin_c_agent.cred_type == CRED_FORMAT_JSON_LD:
            uni_admin_c_agent.public_did = True
            await uni_admin_c_agent.initialize(the_agent=agent)
            log_msg("Admin agent initialized successfully")
        else:
            raise Exception("Invalid credential type:" + str(uni_admin_c_agent.cred_type))

        # Generate invitation for registrar to connect (similar to uni_reg_c pattern)
        log_msg("Generating invitation for registrar connection...")
        try:
            invitation = await uni_admin_c_agent.generate_invitation(
        ...
        if uni_admin_c_agent.show_timing:
            timing = await uni_admin_c_agent.agent.fetch_timing()
            if timing:
                for line in uni_admin_c_agent.agent.format_timing(timing):
                    log_msg(line)

    finally:
        terminated = await uni_admin_c_agent.terminate()
...
if __name__ == "__main__":
    parser = arg_parser(ident="uni_admin_c", port=8120)
