...
class UniRegBAgent(AriesAgent):
    ...
    def generate_approval_request(self, student_data):
        ...
        request = {
            ...
            "registrar_id": "REG002"  # UniRegB identifier
        }
...
async def main(args):
    ...
    try:
        log_status(
            ...
            + (
                f" (Wallet type: {uni_reg_b_agent.wallet_type})"
                if uni_reg_b_agent.wallet_type
                else ""
            )
        )
        agent = UniRegBAgent(
            "uni_reg_b.agent",
            uni_reg_b_agent.start_port,
            uni_reg_b_agent.start_port + 1,
            genesis_data=uni_reg_b_agent.genesis_txns,
            genesis_txn_list=uni_reg_b_agent.genesis_txn_list,
            no_auto=uni_reg_b_agent.no_auto,
            tails_server_base_url=uni_reg_b_agent.tails_server_base_url,
            revocation=uni_reg_b_agent.revocation,
            timing=uni_reg_b_agent.show_timing,
            multitenant=uni_reg_b_agent.multitenant,
            mediation=uni_reg_b_agent.mediation,
            wallet_type=uni_reg_b_agent.wallet_type,
            seed=uni_reg_b_agent.seed,
            aip=uni_reg_b_agent.aip,
            endorser_role=uni_reg_b_agent.endorser_role,
            anoncreds_legacy_revocation=uni_reg_b_agent.anoncreds_legacy_revocation,
            log_file=uni_reg_b_agent.log_file,
            log_config=uni_reg_b_agent.log_config,
            log_level=uni_reg_b_agent.log_level,
            reuse_connections=uni_reg_b_agent.reuse_connections,
            multi_use_invitations=uni_reg_b_agent.multi_use_invitations,
            public_did_connections=uni_reg_b_agent.public_did_connections,
            extra_args=extra_args,
        )

        uni_reg_b_schema_name = "university_registration_schema"
        uni_reg_b_schema_attrs = [
            "student_id",
            "student_name",
            "university_name",
            "graduation_year",
        ...
        if uni_reg_b_agent.cred_type == CRED_FORMAT_INDY:
            uni_reg_b_agent.public_did = True
            await uni_reg_b_agent.initialize(
                the_agent=agent,
                schema_name=uni_reg_b_schema_name,
                schema_attrs=uni_reg_b_schema_attrs,
                create_endorser_agent=(
                    (uni_reg_b_agent.endorser_role == "author")
                    if uni_reg_b_agent.endorser_role
                    else False
                ),
            )
            ...
            agent.cred_def_id = uni_reg_b_agent.cred_def_id
            log_msg(f"Set agent cred_def_id: {agent.cred_def_id}")
            
        elif uni_reg_b_agent.cred_type == CRED_FORMAT_JSON_LD:
            uni_reg_b_agent.public_did = True
            await uni_reg_b_agent.initialize(the_agent=agent)
            # Set the credential definition ID for JSON-LD as well (if available)
            if hasattr(uni_reg_b_agent, 'cred_def_id'):
                agent.cred_def_id = uni_reg_b_agent.cred_def_id
                log_msg(f"Set agent cred_def_id: {agent.cred_def_id}")
        else:
            raise Exception("Invalid credential type:" + uni_reg_b_agent.cred_type)

        # Generate an invitation for students
        await uni_reg_b_agent.generate_invitation(
            display_qr=True,
            reuse_connections=uni_reg_b_agent.reuse_connections,
            multi_use_invitations=uni_reg_b_agent.multi_use_invitations,
            public_did_connections=uni_reg_b_agent.public_did_connections, 
        ...
        if uni_reg_b_agent.endorser_role and uni_reg_b_agent.endorser_role == "author":
            options += "    (D) Set Endorser's DID\n"
        if uni_reg_b_agent.multitenant:
            options += "    (W) Create and/or Enable Wallet\n"
        options += "    (T) Toggle tracing on credential/proof exchange\n"
        options += "    (X) Exit?\n[1/2/3/4/5/6/7/{}{}T/X] ".format(
            "8/9/10/11/" if uni_reg_b_agent.revocation else "",
            "W/" if uni_reg_b_agent.multitenant else "",
        )
        ...
        async for option in prompt_loop(options):
            ...
            if option is None or option in "xX":
            ...
            elif option in "dD" and uni_reg_b_agent.endorser_role:
                endorser_did = await prompt("Enter Endorser's DID: ")
                await uni_reg_b_agent.agent.admin_POST(
                    f"/transactions/{uni_reg_b_agent.agent.connection_id}/set-endorser-info",
                    params={"endorser_did": endorser_did},
                )

            elif option in "wW" and uni_reg_b_agent.multitenant:
                target_wallet_name = await prompt("Enter wallet name: ")
                include_subwallet_webhook = await prompt(
                    "(Y/N) Create sub-wallet webhook target: "
                )
                if include_subwallet_webhook.lower() == "y":
                    created = await uni_reg_b_agent.agent.register_or_switch_wallet(
                        target_wallet_name,
                        webhook_port=uni_reg_b_agent.agent.get_new_webhook_port(),
                        public_did=True,
                        mediator_agent=uni_reg_b_agent.mediator_agent,
                        endorser_agent=uni_reg_b_agent.endorser_agent,
                        taa_accept=uni_reg_b_agent.taa_accept,
                    )
                else:
                    created = await uni_reg_b_agent.agent.register_or_switch_wallet(
                        target_wallet_name,
                        ...
                        mediator_agent=uni_reg_b_agent.mediator_agent,
                        endorser_agent=uni_reg_b_agent.endorser_agent,
                        cred_type=uni_reg_b_agent.cred_type,
                        taa_accept=uni_reg_b_agent.taa_accept,
                    )
                if created:
                    await uni_reg_b_agent.create_schema_and_cred_def(
                        schema_name=uni_reg_b_schema_name,
                        schema_attrs=uni_reg_b_schema_attrs,
                    )
                ...
                if uni_reg_b_agent.aip == 10:
                    proof_request_web_request = (
                        agent.generate_proof_request_web_request(
                            uni_reg_b_agent.aip,
                            uni_reg_b_agent.cred_type,
                            uni_reg_b_agent.revocation,
                            exchange_tracing,
                        )
                    )
                    ...
                    if uni_reg_b_agent.cred_type == CRED_FORMAT_INDY:
                        ...
                        raise Exception(
                            "Error invalid credential type:" + uni_reg_b_agent.cred_type
                        )
                    await agent.admin_POST(
                        "/present-proof-2.0/send-request", proof_request_web_request
                    )
                else:
                    raise Exception(f"Error invalid AIP level: {uni_reg_b_agent.aip}")
                ...
                try:
                    ...
                    if "services" in invitation and len(invitation["services"]) > 0:
                        ...
                        if "serviceEndpoint" in service:
                            ...
                            if "localhost" in endpoint:
                                ...
                                container_endpoint = endpoint.replace("localhost", "uni_admin_b")
                    ...
                    log_msg("3. Verify admin agent is using --port 8100")
            ...
            elif option == "8" and uni_reg_b_agent.revocation:
            ...
            elif option == "9" and uni_reg_b_agent.revocation:
                try:
            ...
            elif option == "10" and uni_reg_b_agent.revocation:
                try:
                    is_anoncreds = agent.__dict__["wallet_type"] == "askar-anoncreds"
                    endpoint = (
                        f"/anoncreds/revocation/active-registry/{uni_reg_b_agent.cred_def_id}/rotate"
                        if is_anoncreds
                        else f"/revocation/active-registry/{uni_reg_b_agent.cred_def_id}/rotate"
                    )
                    resp = await agent.admin_POST(endpoint, {})
                    agent.log(
                        "Rotated registries for {}. Decommissioned Registries: {}".format(
                            uni_reg_b_agent.cred_def_id,
            ...
            elif option == "11" and uni_reg_b_agent.revocation:
                is_anoncreds = agent.__dict__["wallet_type"] == "askar-anoncreds"
                if is_anoncreds:
        ...
        if uni_reg_b_agent.show_timing:
        ...
        terminated = await uni_reg_b_agent.terminate()
...
if __name__ == "__main__":
    parser = arg_parser(ident="uni_reg_b", port=8090)
